...
const QuestionScroll = (props) => {
    const questions = [
        "Are you able to see the screen clearly without loss of vision?",
        ...
        <div id="question-scroll" className="question-scroll">
            {questions.map((value, index) => {
                if (index + 1 == props.questionIndex) {
                    return (
                        <div className="question-piece bold" id={`q-${index + 1}`} key={index}>
                            {index + 1}. {value}
                        </div>
                    )
                } else {
                    return (
                        <div className="question-piece" id={`q-${index + 1}`} key={index}>
                            {index + 1}. {value}
                        </div>
                    )
                }
